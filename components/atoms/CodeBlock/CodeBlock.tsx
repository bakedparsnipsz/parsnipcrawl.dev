...
function highlightRust(code: string): string {
  const KEYWORDS = /\b(fn|let|mut|pub|use|mod|struct|enum|impl|trait|for|in|if|else|match|return|self|Self|super|where|type|const|static|async|await|move|ref|loop|while|break|continue|true|false|Some|None|Ok|Err)\b/g;
  const STRINGS  = /("(?:[^"\\]|\\.)*")/g;
  const COMMENTS = /(\/\/[^\n]*|\/\*[\s\S]*?\*\/)/g;
