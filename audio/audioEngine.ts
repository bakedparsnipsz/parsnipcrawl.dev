...
function startDungeonTrack(): void {
  ...
  const melody: number[] = [
    ...
    N.A4,
    N.B4,
    N.G4, // B natural = the whimsical lift
    N.F4,
    N.A4,
    N.D4,
    N.E4,
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  (droneOsc as any).__setRunning = (v: boolean) => {
    isRunning = v;
  };
}
...
function startCombatTrack(): void {
  ...
  const bassLine: number[] = [N.D2, N.D2, N.A2, N.C3, N.D2, N.D2, N.Bb2, N.A2];
  const melody: number[] = [N.D4, N.F4, N.A4, N.F4, N.D4, N.C4, N.Bb3, N.A3];
  const counter: number[] = [N.A4, N.A4, N.Bb4, N.A4, N.F4, N.A4, N.G4, N.F4];
  ...
  function tick(): void {
    ...
    // Lead melody
    scheduleOsc('square', melody[s], 0.12, dest, now, 0.2);
    ...
    if (step % 4 === 0) {
      ...
      scheduleOsc('sine', N.D2, 0.22, dest, now, 0.07, { targetFreq: N.D2 * 0.4, slideTime: 0.06 });
    }
...
function startBossTrack(): void {
  ...
  const subBass: number[] = [N.D2, N.D2, N.C3, N.D2, N.Bb2, N.D2, N.A2, N.D2];
  const arp1: number[] = [N.D4, N.F4, N.A4, N.D5, N.A4, N.F4, N.Eb4, N.F4];
  const arp2: number[] = [N.D3, N.F3, N.A3, N.C4, N.Bb3, N.A3, N.Gb3, N.A3];
  const lead: number[] = [N.A4, N.A4, N.Bb4, N.A4, N.Gb4, N.A4, N.E4, N.A4];
  ...
  function tick(): void {
    ...
    scheduleOsc('sine', subBass[s], 0.18, dest, now, 0.2);
    // Arp voices
    scheduleOsc('square', arp1[s], 0.1, dest, now, 0.16);
    scheduleOsc('square', arp2[s], 0.09, dest, now, 0.2);
...
function playSfx(name: SfxName): void {
  ...
  switch (name) {
    ...
    case 'xp_small':
      scheduleOsc('sine', N.G4, 0.22, dest, t, 0.1);
    ...
    case 'room_clear': {
      ...
      motif.forEach((n, i) =>
        scheduleOsc('square', n, 0.17, dest, t + i * 0.09, i === motif.length - 1 ? 0.55 : 0.1),
      );
      scheduleOsc('triangle', N.D5 * 2, 0.1, dest, t + 0.46, 0.45);
      break;
    }
    ...
    case 'loot':
      ...
      scheduleNoise(dest, t, 0.1, 0.07, 7000, 'highpass');
      break;
    ...
    case 'combat_hit':
      ...
      scheduleOsc('sine', N.A4, 0.12, dest, t + 0.04, 0.1);
      break;
    ...
    case 'encounter':
      ...
      scheduleOsc('square', N.D3, 0.28, dest, t + 0.1, 0.08);
      scheduleNoise(dest, t + 0.1, 0.16, 0.22, 900, 'bandpass');
      break;

    // ── Footstep: soft dungeon tile ──
    case 'footstep':
      scheduleNoise(dest, t, 0.055, 0.1, 380, 'lowpass');
    ...
    case 'death': {
      ...
      doomed.forEach((n, i) => scheduleOsc('sawtooth', n, 0.18, dest, t + i * 0.13, 0.18));
